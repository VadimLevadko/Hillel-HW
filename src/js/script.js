...
const result = someArr.reduce((acc, item) => {
  if (item > 0) {
    acc.sum += item;
    acc.countPositiveNum++;

    if (item % 2 !== 0) {
      acc.oddNum.count++;
      acc.pairedNum.sum += item;
    } else {
      acc.pairedNum.count++;
      acc.oddNum.sum += item;
    }

    acc.productOfPositiveEl *= item;

    return acc;
  }
  acc.countNegativeNum++;
...
console.log(`Кількість позитивних елементів - ${result.countPositiveNum}. Сума всіх позитивних чисел - ${result.sum}`);
